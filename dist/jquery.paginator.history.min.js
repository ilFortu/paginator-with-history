 ...
 */
!function($){$.fn.paginatorHistory=function(options){function ajaxCall(page,isNext){page=parseInt(page),$.ajax({"url":setUrlwithPagination(ajaxUrl,page),"context":document.body}).done(function(content){settings.ajaxDoneBeforeItemsInPage.call(this);var html=content,removeLink=!1;if(null!=content&&""!=content)if(null!=settings.selectorContainer&&(html=$(content).find(settings.selectorContainer).html()),"undefined"!=typeof html&&""!=html){var newPageBlock="<"+settings.blockPageHtmlElement+" class='"+listItemsPage+"' data-page='"+page+"'>"+html+"</"+settings.blockPageHtmlElement+">";if(isNext)$paginatorListContainer.append(newPageBlock),loadingNext=!1,$next.attr("data-page",page+1);else{$paginatorListContainer.prepend(newPageBlock);var scrollTop=window.pageYOffset,scrollTo=$paginatorListContainer.find(listItemsPageClass+"[data-page='"+currentPage+"']").position().top;if(0!=scrollTop){var heightPrevPage=$paginatorListContainer.find(listItemsPageClass+"[data-page='"+page+"']").height();scrollTo=window.pageYOffset+heightPrevPage}window.scrollTo(0,scrollTo),console.log(scrollTo),page>1?(loadingPrev=!1,$prev.attr("data-page",page-1)):removeLink=!0}}else removeLink=!0;else removeLink=!0;removeLink&&(isNext?$next.remove():$prev.remove()),settings.ajaxDoneAfterItemsInPage.call(this)})}function changeLocation(page,itemId){var newLocation=setUrlwithPagination(window.location.href,page);"undefined"!=typeof itemId&&null!=itemId&&(newLocation+="#"+itemId),history.replaceState(null,null,newLocation)}function setUrlwithPagination(url,page){if(url.indexOf(settings.paramPage+"=")<0)return url+(url.split("?")[1]?"&":"?")+settings.paramPage+"="+page;var regex=new RegExp("("+settings.paramPage+"=).*?((&)(.)*)?$");return url.replace(regex,"$1"+page+"$2")}var defaults={"paramPage":"page","offsetCallPage":200,"selectorContainer":null,"blockPageHtmlElement":"div","ajaxDoneBeforeItemsInPage":function(){},"ajaxDoneAfterItemsInPage":function(){}},settings=$.extend({},defaults,options),$prev=this.find(".jph-prev"),$next=this.find(".jph-next");if($prev.length>0||$next.length>0){var $paginatorListContainer=this.find(".jph-list-container"),ajaxUrl=this.data("jph-ajax-url"),listItemsPage="jph-list-items-page",listItemsPageClass="."+listItemsPage,currentPage=this.find(listItemsPageClass).data("page"),loadingNext=!1,loadingPrev=!1;$prev.length>0&&$prev.attr("data-page")>0&&(loadingPrev=!0,ajaxCall($prev.attr("data-page"),!1)),$(window).scroll(function(){var scrollPos=$(window).scrollTop();$next.length>0&&!loadingNext&&$next.position().top-settings.offsetCallPage<scrollPos+$(window).height()&&(loadingNext=!0,console.log("NEXT"),ajaxCall($next.attr("data-page"),!0)),$prev.length>0&&!loadingPrev&&scrollPos-$prev.position().top<settings.offsetCallPage&&(loadingPrev=!0,console.log("PREV"),ajaxCall($prev.attr("data-page"),!1));var scrollHalfPosition=$(window).scrollTop()+$(window).height()/2;$(listItemsPageClass).each(function(){var top=$(this).position().top,bottom=$(this).position().top+$(this).height();$(this).attr("data-page")!=currentPage&&scrollHalfPosition>=top&&bottom>=scrollHalfPosition&&(currentPage=$(this).attr("data-page"),changeLocation(currentPage,null))})}),$(window).trigger("scroll")}}}(jQuery);
